-- ============================================================
--  core_gps_advanced | Server Main  (ESX-Legacy edition)
--  All QBCore calls replaced with ESX / ox_inventory
--  equivalents. Every command, event, and feature is
--  preserved exactly as the original QBCore version.
-- ============================================================
...
-- ============================================================
--  Version checker (unchanged from original)
-- ============================================================
...
local function CompareVersions(current, latest)
    ...
    if l.major > c.major then return 'outdated' elseif l.major < c.major then return 'ahead' end
    if l.minor > c.minor then return 'outdated' elseif l.minor < c.minor then return 'ahead' end
    if l.patch > c.patch then return 'outdated' elseif l.patch < c.patch then return 'ahead' end
    return 'current'
end
...
-- ============================================================
--  Helper: ESX player name  →  used for GPS ID generation
--  ESX stores character names differently depending on the
--  server setup; we fall back to the FiveM player name so
--  this works regardless of ESX flavour.
-- ============================================================

local function GetESXPlayerName(xPlayer)
    -- Try ESX multi-character / character name fields first
    local ok, firstName = pcall(function() return xPlayer.get('firstName') end)
    local ok2, lastName = pcall(function() return xPlayer.get('lastName')  end)
    if ok and ok2 and firstName and lastName then
        return (firstName .. '_' .. lastName):upper():gsub('%s+', '_')
    end
    -- Fallback: FiveM server-side player name
    return tostring(GetPlayerName(xPlayer.source) or 'PLAYER'):upper():gsub('%s+', '_')
end

-- ============================================================
--  Helper: ESX notification shorthand
-- ============================================================

local function Notify(source, message, ntype)
    -- ESX Legacy native client notification event
...
function GenerateGPSId(playerName)
    ...
    local result = exports['oxmysql']:executeSync(
        'SELECT COUNT(*) as count FROM core_gps_advanced_devices WHERE gps_id = ?', {id})
    if result and result[1] and result[1].count > 0 then
        return GenerateGPSId(playerName)   -- collision – try again
    end
    return id
end

-- ============================================================
--  Admin command: /givegpsa
--  ESX.RegisterCommand replaces QBCore.Commands.Add
-- ============================================================

ESX.RegisterCommand('givegpsa', 'admin', function(xPlayer, args, showError)
    local src        = xPlayer.source
    local playerName = GetESXPlayerName(xPlayer)
    local gpsId      = GenerateGPSId(playerName)

    exports['oxmysql']:insertSync(
        'INSERT INTO core_gps_advanced_devices (gps_id) VALUES (?)', {gpsId})

    -- ox_inventory: add item with metadata containing the GPS ID
    exports.ox_inventory:AddItem(src, Config.ItemName, 1, { gps_id = gpsId })

    Notify(src, 'GPS Device ID: ' .. gpsId)
end, false, {})

-- ============================================================
--  Register device event (triggered from client if needed)
-- ============================================================
...
-- ============================================================
--  Load markers for a GPS device
-- ============================================================

RegisterNetEvent('core_gps:server:loadMarkers', function(gpsId)
    local src     = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer or not gpsId then return end

    local result = exports['oxmysql']:executeSync(
        'SELECT * FROM core_gps_advanced WHERE gps_id = ? ORDER BY id ASC', {gpsId})

    local deviceResult = exports['oxmysql']:executeSync(
        'SELECT allow_receive_locations FROM core_gps_advanced_devices WHERE gps_id = ?', {gpsId})
...
RegisterNetEvent('core_gps:server:addMarker', function(gpsId, markerData)
    local src     = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer or not gpsId or not markerData then return end

    if not gpsMarkers[gpsId] then gpsMarkers[gpsId] = {} end

    if #gpsMarkers[gpsId] >= Config.MaxMarkers then
        Notify(src, 'This GPS has reached the maximum number of markers (' .. Config.MaxMarkers .. ')')
        return
    end

    local insertId = exports['oxmysql']:insertSync(
        'INSERT INTO core_gps_advanced (gps_id, label, coords, street, timestamp) VALUES (?, ?, ?, ?, ?)',
        {gpsId, markerData.label, json.encode(markerData.coords), markerData.street, markerData.timestamp})

    if insertId then
        markerData.id = insertId
        table.insert(gpsMarkers[gpsId], markerData)
        TriggerClientEvent('core_gps:client:updateMarkers', src, gpsMarkers[gpsId])
        Notify(src, 'Location marked!')
    else
        Notify(src, 'Failed to save marker')
    end
end)
...
RegisterNetEvent('core_gps:server:removeMarker', function(gpsId, index)
    ...
    if gpsMarkers[gpsId] and gpsMarkers[gpsId][index] then
        ...
        exports['oxmysql']:executeSync(
            'DELETE FROM core_gps_advanced WHERE id = ? AND gps_id = ?', {markerId, gpsId})
        table.remove(gpsMarkers[gpsId], index)
        TriggerClientEvent('core_gps:client:updateMarkers', src, gpsMarkers[gpsId])
        Notify(src, 'Marker removed!')
    end
end)
...
RegisterNetEvent('core_gps:server:shareMarker', function(gpsId, targetId, markerIndex)
    ...
    if gpsMarkers[gpsId] and gpsMarkers[gpsId][markerIndex] then
        ...
        local senderName  = GetPlayerName(src)   -- ESX-safe name for UI display
...
-- ============================================================
--  Notify sender that receiver rejected the share
-- ============================================================
...
RegisterNetEvent('core_gps:server:notifyShareRejected', function(senderSource)
    ...
    local receiverName = GetPlayerName(src)
    Notify(senderSource, receiverName .. ' is not accepting shared locations.')
end)
...
RegisterNetEvent('core_gps:server:renameMarker', function(gpsId, index, newLabel)
    local src     = source
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer or not gpsId or not newLabel then return end

    if gpsMarkers[gpsId] and gpsMarkers[gpsId][index] then
        local markerId = gpsMarkers[gpsId][index].id
        exports['oxmysql']:executeSync(
            'UPDATE core_gps_advanced SET label = ? WHERE id = ? AND gps_id = ?',
            {newLabel, markerId, gpsId})
        gpsMarkers[gpsId][index].label = newLabel
        TriggerClientEvent('core_gps:client:updateMarkers', src, gpsMarkers[gpsId])
        Notify(src, 'Location renamed!')
    end
end)

-- ============================================================
--  Update receive-locations setting
-- ============================================================
...
RegisterNetEvent('core_gps:server:updateReceiveSetting', function(gpsId, allowed)
    ...
    exports['oxmysql']:executeSync(
        'UPDATE core_gps_advanced_devices SET allow_receive_locations = ? WHERE gps_id = ?',
        {allowValue, gpsId})
end)
